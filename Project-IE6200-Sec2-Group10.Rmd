...
```{r}
pmf_df <- data_science_df %>%
    filter(CASE_STATUS == "CERTIFIED") %>% 
    group_by(YEAR) %>%
    summarise(count=n(),  .groups = 'drop') %>%
    mutate(pmf = count/sum(count))

ggplot(pmf_df, aes(count, pmf)) +
  geom_bar(stat="identity", fill="steelblue")+
  theme_bw() +
  ggtitle("PMF for Accepted applications from 2011-2016")+
  labs(x = 'No of accepted applications', y = 'Count') + 
  theme(plot.title = element_text(hjust = 0.5)) + 
  scale_x_continuous("Count of accepted applicatin", labels = as.character(pmf_df$count),breaks = pmf_df$count)
```
...
ggplot(cdf_df, aes(x=count, y=cdf))+
  geom_bar(stat="identity", fill="steelblue")+
  theme_bw() +
  ggtitle("CDF for Accepted applications from 2011-2016")+
  labs(x="Number of accepted applications", y="CDF of accepted applications")+
...
```
```{r echo=FALSE}
fit_x <- fitdist(wage_sample$PREVAILING_WAGE, "norm")
summary(fit_x)
```
```{r echo=FALSE}
